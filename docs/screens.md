...
- 主タイトル「いまここインフォ」
- 英字表記「imacoco-info」は主タイトルと左右端が概ね揃うよう、画面幅に応じて字間を調整する
- 主タイトル直下に小さく控えめな「© 2026 SIKUMI LAB」
- 現在日付「YYYY年M月D日（曜）」と現在時計「HH:mm」。当日が日本の祝日・休日なら、広い画面では祝日名、520px以下では名称を読み上げ可能な「祝」バッジを日付の隣に表示する
- 現在日付と時計は同じ文字サイズ・太さにする
- 410px前後のスマートフォン幅では日付・時計を18px以上で表示し、テーマ選択の下端から日付カードまでと、日付カードから現在地カードまでの間隔を同程度に揃える
- 祝日表示を追加しても日付と時計を一行に保ち、日付カードの横スクロールや幅超過を発生させない
...
| C-002 | 天気 | 天気、現在気温、体感、最高／最低、最大1時間予想降水量、6時間予報の開閉 | FR-005 |
...
- 体感、最高、最低、最大雨量の補足値は通常のスマートフォン幅で横一列にし、値の縦位置を揃える。
- 現在気温はスマートフォン幅では36px、PC幅では40pxを基準とする。スマートフォン幅では気温の数値と単位だけを通常位置から2px下げ、天気状態の文字位置は変えない。
- 直近6時間は開閉式。各時刻に気温、簡潔な天気状態（晴れ、くもり、雨、雪、雷雨）、直前1時間の予想降水量を`0.0mm`形式で併記する。6件を時系列の小カードで横並びにし、狭い画面ではカード内だけを横スクロールできるようにする。
- 今日の時間別予想降水量の最大値を「最大雨量」「0.0mm」形式で表示し、「※今日の最大1時間予想降水量です。」を天気カードのタイトル行右側へ表示する。天気データの未取得・取得中・失敗時には表示しない。320px幅でもタイトルと同じ行に収め、縦中央を揃える。6時間予報の展開時だけ、その時間別一覧の下に「※時間別の降水量は直前1時間の予想値です。」を表示する。どちらも補足情報の青色と共通注釈サイズを基準にする。
- 取得時刻と予報対象時刻を分ける。

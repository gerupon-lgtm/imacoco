...
- 写真: 使用しない
- アイコン: 外部素材や絵文字へ依存しないアプリ内カラーSVGで統一する。天気・太陽・潮・駅・役所・医療・主要操作の意味を補助し、必ず文字ラベルを併記
...
| C-008 | 最寄り駅 | 最寄り1駅、次候補2駅の開閉、路線、運営会社、距離、方角、地図 | FR-016 |
| C-005 | 役所 | 都道府県庁、管轄役所、市役所補助導線【想定】、距離、方角、公式／地図 | FR-008 |
...
- 天気アイコンと日本語ラベルを併記する。
- 直近6時間は開閉式。横スクロール表ではなく、狭い画面では2〜3列の小カードまたは縦リストにする。
- 取得時刻と予報対象時刻を分ける。
- 日の出・日の入りはJSTの24時間表記とする。
...
- モデル格子が30km超ならカードを表示しない。

### 6.7 最寄り駅カード

- 最寄り1駅を最初から表示し、「ほかの駅を見る」で次候補2駅を展開する。
- 各行: 駅名、路線名、運営会社、直線距離、方角、「地図で開く」。
...
- 住所、電話、開庁時間は表示しない。
- 【想定】政令指定都市の区では区役所を主表示、市役所を「市役所も見る」として補助表示する。
...
- 「現在診療中」「救急受入可能」を表示しない。
- カード末尾に「緊急時は119」「受診前に公式情報で確認」を固定表示する。
- 読み込み状況と「緊急時は119」は同一行へ配置し、狭幅時だけ折り返す。
...
- 潮の目安以降が最寄り駅、役所、医療機関の順であることをDOM順とキーボード読み上げ順の両方で確認する。

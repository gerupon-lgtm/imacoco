...
- 今日の降水確率には「※降水の％は今日の最大値です。」を天気カードのタイトル行右側へ表示する。天気データの未取得・取得中・失敗時には表示しない。320px幅でもタイトルと同じ行に収め、縦中央を揃える。6時間予報の展開時だけ、その時間別一覧の下に「※時間別の％は直前1時間の降水確率です。」を表示する。どちらも補足情報の青色と共通注釈サイズを基準にする。
- 取得時刻と予報対象時刻を分ける。

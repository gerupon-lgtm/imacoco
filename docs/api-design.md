...
| P-012 | 日本の祝日照合 | 静的JSON＋端末内計算 | FR-004 |
...
1. GitHub Actionsを毎月1日に実行し、内閣府の公式CSVを取得する。
2. Content-Type、Shift_JISデコード、ヘッダー、日付、名称、重複、最低件数を検証する。既存生成物より件数や収録期限が後退する、または既存日付が欠落するデータは拒否する。
3. `YYYY-MM-DD`をキーとする静的JSONを生成し、差分がある場合だけ自動PRを作成する。
4. アプリは通信せず、現在瞬間をJST日付へ変換して同梱JSONを参照する。
5. 祝日は広い画面で名称、520px以下で名称をアクセシブル名に持つ「祝」バッジを表示する。

CSV取得失敗・形式変更・検証失敗時は生成物を更新せず、既存データと本番表示を維持する。自動PRはCI成功後に人が確認してマージする。
